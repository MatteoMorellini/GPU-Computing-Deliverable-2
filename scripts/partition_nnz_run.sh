#!/bin/bash
#SBATCH --job-name=partition_nnz
#SBATCH --output=partition_nnz_%j.out
#SBATCH --error=partition_nnz_%j.err
...
# This executable uses MPI collectives only to prepare partitions; it never
# initializes CUDA or launches a GPU kernel.
export OMPI_MCA_opal_warn_on_missing_libcuda=0

OUTPUT="${OUTPUT:-results/partition_nnz_${SLURM_JOB_ID:-local}.csv}"
PARTITION_SEED="${PARTITION_SEED:-20260722}"
LONG_ROW_FRACTION="${LONG_ROW_FRACTION:-0.35}"

make bin/analyze_partition_nnz
